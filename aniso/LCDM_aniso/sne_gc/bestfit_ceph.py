...
def chi2_joint(H0_p, om_p, M_p, k_p, s_p, sig_p,
...
def fit_joint(idx_sn_p, idx_sn_m, idx_gc_p, idx_gc_m, z_sn, mb, cov, mu_ceph, calibrator, z_gc, sig_L, f, logT_err, tau): 
    ...
    func = lambda H0_p, om_p, M_p, k_p, s_p, sig_p, H0_m, om_m, M_m, k_m, s_m, sig_m: chi2_joint(H0_p, om_p, M_p, k_p, s_p, sig_p, H0_m, om_m, M_m, k_m, s_m, sig_m, idx_sn_p, idx_sn_m, idx_gc_p, idx_gc_m, z_sn, mb, L, mu_ceph, calibrator, z_gc, sig_L, f, logT_err, tau)
    ...
    #errs = np.array([m.errors["H0_p"], m.errors["om_p"], m.errors["M_p"], m.errors["k_p"], m.errors["s_p"], m.errors["sig_p"], m.errors["H0_m"], m.errors["om_m"], m.errors["M_m"], m.errors["k_m"], m.errors["s_m"], m.errors["sig_m"]])
...
if __name__=="__main__":
    ...
    # + hemisphere
         ...
         H0_p[i], om_p[i], M_p[i], k_p[i], s_p[i], sig_p[i], H0_m[i], om_m[i], M_m[i], k_m[i], s_m[i], sig_m[i] = best
         H0_p_err[i], om_p_err[i], M_p_err[i], k_p_err[i], s_p_err[i], sig_p_err[i], H0_m_err[i], om_m_err[i], M_m_err[i], k_m_err[i], s_m_err[i], sig_m_err[i] = err
