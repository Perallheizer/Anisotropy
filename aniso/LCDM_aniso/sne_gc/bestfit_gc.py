...
def chi2_joint(H0_p, om_p, M_p, s_p, sig_p,
    H0_m, om_m, M_m, s_m, sig_m,
    ...
    chi_gc_p = chisq_gc([H0_p, om_p, s_p, sig_p], idx_gc_p, z_gc, sig_L, f, logT_err, tau)
    chi_gc_m = chisq_gc([H0_m, om_m, s_m, sig_m], idx_gc_m, z_gc, sig_L, f, logT_err, tau)
...
def fit_joint(idx_sn_p, idx_sn_m, idx_gc_p, idx_gc_m, z_sn, mb, cov, mu_ceph, calibrator, z_gc, sig_L, f, logT_err, tau): 
    ...
    func = lambda H0_p, om_p, M_p, s_p, sig_p, H0_m, om_m, M_m, s_m, sig_m: chi2_joint(H0_p, om_p, M_p, s_p, sig_p, H0_m, om_m, M_m, s_m, sig_m, idx_sn_p, idx_sn_m, idx_gc_p, idx_gc_m, z_sn, mb, L, mu_ceph, calibrator, z_gc, sig_L, f, logT_err, tau)
       
    m = Minuit(func, H0_p=70, om_p=0.3, M_p=-19.3, s_p=2.102, sig_p=0.24, H0_m=70, om_m=0.3, M_m=-19.3, s_m=2.102, sig_m=0.24)
    m.limits = [(0,150), (0,1), (-20,-18), (1e-4, 5), (1e-4, 2),(0,150), (0,1), (-20,-18), (1e-4, 5), (1e-4, 2)]
    m.errordef = 1

    m.migrad()
    #m.hesse()

    best = np.array([m.values["H0_p"], m.values["om_p"], m.values["M_p"], m.values["s_p"], m.values["sig_p"], m.values["H0_m"], m.values["om_m"], m.values["M_m"], m.values["s_m"], m.values["sig_m"]])
    #errs = np.array([m.errors["H0_p"], m.errors["om_p"], m.errors["M_p"], m.errors["s_p"], m.errors["sig_p"], m.errors["H0_m"], m.errors["om_m"], m.errors["M_m"], m.errors["s_m"], m.errors["sig_m"]])
...
if __name__=="__main__":
    ...
    # + hemisphere
         ...
         H0_p[i], om_p[i], M_p[i], s_p[i], sig_p[i], H0_m[i], om_m[i], M_m[i], s_m[i], sig_m[i] = best
         H0_p_err[i], om_p_err[i], M_p_err[i], s_p_err[i], sig_p_err[i], H0_m_err[i], om_m_err[i], M_m_err[i], s_m_err[i], sig_m_err[i] = err
