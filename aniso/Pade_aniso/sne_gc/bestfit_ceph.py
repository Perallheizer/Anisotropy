...
def chi2_joint(H0_p, q0_p, M_p, k_p, s_p, sig_p,
    H0_m, q0_m, M_m, k_m, s_m, sig_m,
    ...
    chi_gc_p = chisq_gc([H0_p, q0_p, k_p, s_p, sig_p], idx_gc_p, z_gc, sig_L, f, logT_err, tau)
    chi_gc_m = chisq_gc([H0_m, q0_m, k_m, s_m, sig_m], idx_gc_m, z_gc, sig_L, f, logT_err, tau)
...
def fit_joint(idx_sn_p, idx_sn_m, idx_gc_p, idx_gc_m, z_sn, mb, cov, mu_ceph, calibrator, z_gc, sig_L, f, logT_err, tau): 
    ...
    func = lambda H0_p, q0_p, M_p, k_p, s_p, sig_p, H0_m, q0_m, M_m, k_m, s_m, sig_m: chi2_joint(H0_p, q0_p, M_p, k_p, s_p, sig_p, H0_m, q0_m, M_m, k_m, s_m, sig_m, idx_sn_p, idx_sn_m, idx_gc_p, idx_gc_m, z_sn, mb, L, mu_ceph, calibrator, z_gc, sig_L, f, logT_err, tau)
       
    m = Minuit(func, H0_p=70, q0_p=-0.55, M_p=-19.3, k_p = 1.136, s_p=2.102, sig_p=0.24, H0_m=70, q0_m=-0.55, M_m=-19.3, k_m = 1.136, s_m=2.102, sig_m=0.24)
    m.limits = [(0,150), (-2, 2), (-20,-18), (1e-4, 3), (1e-4, 5), (1e-4, 2),(0,150), (-2, 2), (-20,-18), (1e-4, 3), (1e-4, 5), (1e-4, 2)]
    m.errordef = 1

    m.migrad()
    #m.hesse()

    best = np.array([m.values["H0_p"], m.values["q0_p"], m.values["M_p"], m.values["k_p"], m.values["s_p"], m.values["sig_p"], m.values["H0_m"], m.values["q0_m"], m.values["M_m"], m.values["k_m"], m.values["s_m"], m.values["sig_m"]])
    #errs = np.array([m.errors["H0_p"], m.errors["q0_p"], m.errors["M_p"], m.errors["k_p"], m.errors["s_p"], m.errors["sig_p"], m.errors["H0_m"], m.errors["q0_m"], m.errors["M_m"], m.errors["k_m"], m.errors["s_m"], m.errors["sig_m"]])
...
k_p = np.zeros(Npix) # + hemisphere
k_m = np.zeros(Npix) # - hemisphere
...
k_p_err = np.zeros(Npix) # + hemisphere
k_m_err = np.zeros(Npix) # - hemisphere
...
if __name__=="__main__":
    ...
    # + hemisphere
         ...
         H0_p[i], q0_p[i], M_p[i], k_p[i], s_p[i], sig_p[i], H0_m[i], q0_m[i], M_m[i], k_m[i], s_m[i], sig_m[i] = best
         H0_p_err[i], q0_p_err[i], M_p_err[i], k_p_err[i], s_p_err[i], sig_p_err[i], H0_m_err[i], q0_m_err[i], M_m_err[i], k_m_err[i], s_m_err[i], sig_m_err[i] = err
     ...
     df.to_csv(f"{DIR}/results/H0_ceph_{cut_name}.csv", index=False)
